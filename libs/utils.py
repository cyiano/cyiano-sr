...
def compute_psnr(im1, im2, bit, per_channel=True):
    """
    Args:
    ...
    if per_channel is True:
        if im1.ndim == 3:
            ...
            err = 10 * np.mean(np.log10((2**bit-1)*(2**bit-1)/err))
        elif im1.ndim == 4:
            err = np.mean((im1 - im2) ** 2, axis=(1, 2))
            err = 10 * np.mean(np.log10((2**bit-1)*(2**bit-1)/err))
    else:
        err = np.mean((im1 - im2) ** 2)
        err = 10 * np.log10((2**bit-1)*(2**bit-1)/err)

    return err

def bicubic_interpolation_2d(input_, new_size, endpoint=False):
    """
    Args :
